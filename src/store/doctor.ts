...
export function formatHealth(h: IndexHealth): string {
  ...
  // Пороги откалиброваны по двум наблюдениям, а не по интуиции.
  // Деградировавший индекс: среднее 88.6%, на худшем запросе НОЛЬ, ответы теряются.
  // Здоровый (пересобранный): среднее 93.2%, худший запрос 32%, потерь нет.
  // Отсюда порог по худшему — 10%, а не 50%: на здоровом индексе плотная
  // окрестность легко даёт треть совпадений, и порог в 50% поднимал бы тревогу
  // на исправной системе. Доктор, кричащий волки, перестаёт работать.
