...
const ConfigSchema = z.object({
  ...
  chunk: z
    .object({
      ...
      // 500 замерено как оптимум на unitify (RECALL85 §4.5, эксперимент D):
      // при 200 recall@5 = 46.6%, при 300 — 44.8%, при 500 — 51.7%, при 700 —
      // снова 44.8%. Кривая с максимумом, а не край диапазона. Крупные чанки
      // выигрывают по всем осям сразу: и recall, и меньше склейки по файлам,
      // и на 10% меньше векторов — заголовок обогащения на них разбавляется
      // меньше, а сам фрагмент остаётся самодостаточным.
      ...
      // Замерено раздельно на unitify (RECALL85 §4.5): весь выигрыш даёт
      // модульный JSDoc (@1 24.1% → 29.3%, MRR 0.341 → 0.371), а JSDoc класса
      // в отдельности неотличим от его отсутствия (@1 24.1%, MRR 0.343) —
      // при том, что кластеризацию он всё же ухудшает. Поэтому включён первый
      // и выключен второй; флаг оставлен, потому что на объектно-ориентированном
      // корпусе результат может быть иным.
